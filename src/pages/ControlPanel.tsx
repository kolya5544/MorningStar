...
import { Home, LogOut, Shield, Trash2, RefreshCw } from "lucide-react";
...
export default function ControlPanel(): JSX.Element {
  ...
  const load = async () => {
    try {
      ...
      setError(e?.message ?? "Failed to load control panel");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleChangeRole = async (userId: string, nextRole: Role) => {
    try {
      ...
      if (userId === meId) setRole(updated.role);
    } catch (e: any) {
      if (isAuthError(e)) {
        logout();
        return;
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-zinc-400">{role === "admin" ? "Admin" : "Manager"} mode</div>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="flex justify-end">
                        <select
                          ...
                          onChange={(e) => void handleChangeRole(user.id, e.target.value as Role)}
                        >
